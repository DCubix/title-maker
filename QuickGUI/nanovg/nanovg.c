									...
									float aa, float u0, float u1)
{
	...
	nvg__vset(dst, px - dlx*w - dx*aa, py - dly*w - dy*aa, u1,0, 0, 1, -aa); dst++;
...
static int nvg__expandStroke(NVGcontext* ctx, float w, float fringe, int lineCap, int lineJoin, int lineStyle, float miterLimit)
{
	...
	for (i = 0; i < cache->npaths; i++) {
		...
		if (lineStyle > 1) cverts += 4 * path->count;
		if (loop == 0) {
			// space for caps
	...
	for (i = 0; i < cache->npaths; i++) {
		...
		if (loop) {
			// Loop it
			nvg__vset(dst, verts[0].x, verts[0].y, u0,1, 1, -1, t); dst++;
			nvg__vset(dst, verts[1].x, verts[1].y, u1,1, 1, 1, t); dst++;
		} else {
			// Add cap
			dx = p1->x - p0->x;
			dy = p1->y - p0->y;
